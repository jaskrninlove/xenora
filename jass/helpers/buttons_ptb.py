...
def pbtn(text: str, *, callback_data=None, url=None, style=None, icon_name=None) -> Btn:
    ...
    return Btn(text=text, callback_data=callback_data, url=url, api_kwargs=api_kwargs or None)


def start_buttons(username: str = None):
    bot_username = username or "LunariaMusicBot"
    return Markup([
        [pbtn("Add Me In Your Chat", url=f"https://t.me/{bot_username}?startgroup=true", style="success", icon_name="add")],
        [pbtn("Help & Commands", callback_data="help:main", style="primary", icon_name="music")],
        [
            pbtn("Support", url=SUPPORT_URL, style="primary", icon_name="support"),
            pbtn("Updates", url=UPDATES_URL, style="primary", icon_name="updates"),
        ],
        [pbtn("Owner", url=OWNER_URL, style="danger", icon_name="owner")],
    ])
...
def help_buttons(page: str = "main"):
    return Markup([
        [
            ...
            pbtn("Admin", callback_data="help:admin", style="primary", icon_name="settings"),
        ],
        [
...
def group_start_buttons(username: str):
    bot_username = username or "LunariaMusicBot"
    return Markup([
        [pbtn("Open Private", url=f"https://t.me/{bot_username}?start=group", style="success", icon_name="bot")],
        [
            pbtn("Support", url=SUPPORT_URL, style="primary", icon_name="support"),
            pbtn("Updates", url=UPDATES_URL, style="primary", icon_name="updates"),
        ],
